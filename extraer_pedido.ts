...
function matchProducto(
  ...
  /* ── LO QUE DIJO EL CLIENTE MANDA SOBRE LO QUE DIJO EL MODELO ──────────
     Caso real (3-ago): el cliente escribio "una salchipapa con pollo con
     adicion de salchicha ranchera" y el modelo devolvio "Premium" tipo pollo:
     $28.000 en vez de $17.000. La comprobacion de mas abajo no lo atrapaba
     porque buscaba un producto llamado como EL MODELO ("Premium"), lo
     encontraba, y lo daba por bueno.
     Esta mira lo unico que no se inventa: las palabras del cliente. Si el
     producto elegido tiene alguna palabra que el cliente nunca escribio, y
     existe otro de la misma familia cuyo nombre esta COMPLETO en su texto, ese
     gana. Con tolerancia a como escribe la gente ("premio" y "premiun" cuentan
     como "premium"), que fue lo que rompio el primer intento. */
  const cerca = (a: string, b: string) => {
    if (a === b) return true;
    if (Math.abs(a.length - b.length) > 2) return false;
    if (a.length >= 5 && b.length >= 5 && a.slice(0, 4) !== b.slice(0, 4)) return false;
  ...
  if (matched) {
    const palabrasCli = cliBlob.split(/\s+/).filter(Boolean);
    const dijo = (w: string) => palabrasCli.some(x => cerca(w, x));
    /* DOS VARAS DISTINTAS, y la diferencia importa:
        · para juzgar si al producto ELEGIDO le sobran palabras se admite que el
          cliente escriba mal ("premio" cuenta como "premium"): no se le puede
          castigar por una letra.
        · pero para PROPONER otro producto se exige la palabra EXACTA. Sin esto,
          en tu carta "PREMIO 1.5 LITROS" (gaseosa) se daba por dicho porque el
          cliente habia escrito "premium" hablando de la salchipapa, y la
          Postobon Colombiana se convertia en un Premio. */
    const dijoExacto = (w: string) => palabrasCli.includes(w);
    const sinDecir = (p: Record<string, unknown>) =>
      norm(String(p.name || "")).split(/\s+/).filter(w => w.length >= 4 && !dijo(w));
    const sinDecirExacto = (p: Record<string, unknown>) =>
      norm(String(p.name || "")).split(/\s+/).filter(w => w.length >= 4 && !dijoExacto(w));
    if (sinDecir(matched).length) {
      const raizDe = (p: Record<string, unknown>) =>
        norm(String((catsById.get(String(p.category_id)) || {}).name || "")).split(/\s+/)[0] || "";
      const miRaiz = raizDe(matched);
      const dijoEspecial = dijo("especial") || dijo("especiales");
      let mejor: Record<string, unknown> | null = null;
      for (const p of (allProducts || [])) {
        if (p === matched || esAdicion(p)) continue;
        /* No sirve como candidato lo que el cliente pidio como ADICION. En
           "una salchipapa con pollo con adicion de salchicha ranchera" la
           palabra "ranchera" esta en su texto, pero es la adicion, no el
           plato: sin esto se cambiaba Pollo por RANCHERA. */
        if (adicTexto && norm(adicTexto).includes(norm(String(p.name || "")))) continue;
        if (raizDe(p) !== miRaiz) continue;
        const pn = norm(String(p.name || ""));
        if (!pn || pn.length < 4 || sinDecirExacto(p).length) continue;
        if (dijoEspecial && !/especial/.test(pn)) continue;
        if (!mejor || pn.length > norm(String(mejor.name || "")).length) mejor = p;
      }
      if (mejor) { matched = mejor; prodPorNumero = mejor; bestScore = 99; }
    }
  }
...
- SALSAS — PREFERENCIA vs ADICIÓN. Míralo en este orden y no te saltes el paso 1:
  PASO 1. ¿Aparece junto a la salsa la palabra "adición", "adicional", "extra", "aparte" o "más"? ENTONCES ES UN PRODUCTO, sin excepción: búscalo en el MENÚ y ponlo como una línea más. Ejemplos que SIEMPRE son producto: "una adición de salsa de ajo", "salsa adicional aparte", "extra de salsa".
  PASO 2. Solo si NO aparece ninguna de esas palabras, mira si está diciendo con cuáles salsas quiere el plato. Todo plato ya viene con roja, ajo, rosada y BBQ, así que "solo ajo y bbq", "todas las salsas menos BBQ", "sin salsa de ajo" o "con poquita salsa" son PREFERENCIA: van en "notas", no se cobran.
  En una misma frase pueden ir las dos: "solo ajo y bbq Y UNA ADICIÓN DE SALSA DE AJO" = preferencia en notas + UNA adición como producto.
- CÓMO LLAMA LA GENTE A LOS TAMAÑOS: "de caja", "en caja" o "caja" es el LITRO (el jugo HIT de litro viene en caja y no hay caja pequeña). "de botella" es Personal.
- EL PRECIO IDENTIFICA EL PRODUCTO: muchos clientes no dicen el nombre sino el precio ("de 69", "la de 35", "una maicitos especial de 29"). Busca en el MENÚ cuál cuadra con ese precio, contando tamaño y tipo. El precio que diga puede venir con el empaque sumado: escoge el más cercano por debajo o igual. Si ninguno cuadra, deja "n": null.
...
BARRIOS CONOCIDOS (escribe el barrio EXACTAMENTE como aparece aquí cuando corresponda): ${barriosConocidos.join(", ") || "(ninguno)"}`;
    ...
    /* ═══ LAS TRES PIEZAS DEL DISEÑO DE SERGIO ═══════════════════════════
       Se añaden DESPUES de que produccion armo el pedido, para no tocar ni una
       de sus defensas. Cada agente responde UNA pregunta y ve solo lo suyo.
       ═══════════════════════════════════════════════════════════════════════ */
    const _NL = String.fromCharCode(10);
    const _vozCliente = clienteTexto;
    async function _agente(sis: string, usr: string, tok = 150): Promise<Record<string, unknown>> {
      try {
        const r = await fetch("https://api.openai.com/v1/chat/completions", {
          ...
          body: JSON.stringify({
            model: "gpt-4o-mini",
            messages: [{ role: "system", content: sis }, { role: "user", content: usr }],
            max_tokens: tok, temperature: 0, response_format: { type: "json_object" },
          }),
        });
        if (!r.ok) return {};
    ...
    /* (1) AGENTE DE VARIABLES. Solo entra cuando el producto TIENE variantes y
       quedaron vacias. Es lo unico que arreglo el sabor del jugo: preguntado
       aparte, con cinco opciones delante y una sola pregunta, no se pierde. */
    for (const p of productos) {
      if (!p.matched || !p.product_id) continue;
      const cat = allProducts.find(x => String(x.id) === String(p.product_id));
      const grupos = ((cat?.variables as Array<{ name: string; options: Array<{ name: string }> }>) || [])
        .filter(g => (g.options || []).length);
      const yaTiene = String(p.tipo || "").trim();
      if (!grupos.length || yaTiene) continue;

      const sis = 'El plato ya esta decidido: "' + String(cat?.name || "") + '". Tu UNICA tarea es decir sus variantes. Devuelve un JSON asi: {"tipo":"..."} o {"tipo":null}.' +
        _NL + grupos.map(g => "- " + g.name + ": elige UNA EXACTA de " + (g.options || []).map(o => o.name).join(" | ")).join(_NL) +
        _NL + '- Basta con que el cliente diga una parte: "tropical" es "Frutos tropicales", "naranja" es "Naranja piña".' +
        _NL + "- Si hay varios grupos, llena TODOS separados por coma." +
        _NL + "- Relee todo antes de responder null: pudo decirlo en otro mensaje.";
      const r = await _agente(sis, "LO QUE ESCRIBIO EL CLIENTE:" + _NL + _vozCliente, 100);
      let elegido = String(r.tipo || "").trim();

      // Red de seguridad por codigo: si el agente lo dejo vacio pero la palabra
      // esta en el texto del cliente, se pone sin preguntarle a nadie.
      if (!elegido) {
        const t = _vozCliente.toLowerCase();
        const hall: string[] = [];
        for (const g of grupos) for (const o of (g.options || [])) {
          const on = String(o.name || "").toLowerCase();
          if (on && on.split(/\s+/).some(w => w.length >= 4 && t.includes(w))) { hall.push(String(o.name)); break; }
        }
        elegido = hall.join(", ");
      }
      if (!elegido) continue;

      /* El nombre y el precio se rehacen por el camino de siempre (matchProducto),
         no a mano: el precio nunca lo decide un agente. */
      const rehecho = matchProducto(
        { n: allProducts.findIndex(x => String(x.id) === String(p.product_id)) + 1,
          nombre: String(cat?.name || ""), tamano: p.tamano, tipo: elegido,
          cantidad: p.cantidad, adiciones: ((p.adiciones as ModOpt[]) || []).map(a => a.name).join(", "),
          notas: p.notas },
        allProducts, allMods, allCats, clienteTexto, (cfgDomiRow?.[0] as Record<string, unknown>) || null);
      if (rehecho && rehecho.matched) Object.assign(p, rehecho);
    }

    /* ── (4) UN SABOR QUE SOLO EXISTE EN UN SITIO ─────────────────────────
       Sergio: "absolutamente nadie va a decir la palabra postobon; todo el
       mundo va a decir dame una colombiana. ¿Cuantas cosas en la carta se
       llaman colombiana? no hay manera de equivocarse."
       Exacto. Si el cliente nombra un sabor que en TODO el catalogo pertenece a
       un solo producto, ese producto es — aunque haya dicho "1.5 litros" y eso
       se parezca al nombre de otro (PREMIO 1.5, QUATRO 1.5).

       DOS CANDADOS, porque sin ellos esto se desmadra:
        · el sabor tiene que tener UN SOLO dueño (Mixta esta en Premium y en
          Maicitos Especial: no sirve de pista);
        · y NO puede ser tambien el nombre de una adicion (Tocineta, Carne,
          Pollo, Ranchera... si no, "una premium con tocineta" se convertiria en
          una SUPER QUESO). */
    {
      const nombresAdic = new Set((allMods || []).flatMap(m =>
        ((m.options as Array<{ name: string }>) || []).map(o => String(o.name || "").toLowerCase().trim())));
      const dueno = new Map<string, string[]>();
      for (const pr of allProducts) {
        for (const g of ((pr.variables as Array<{ options?: Array<{ name: string }> }>) || [])) {
          for (const o of (g.options || [])) {
            const k = String(o.name || "").toLowerCase().trim();
            if (!k || nombresAdic.has(k)) continue;
            const l = dueno.get(k) || [];
            if (!l.includes(String(pr.id))) l.push(String(pr.id));
            dueno.set(k, l);
          }
        }
      }
      const voz = " " + _vozCliente.toLowerCase().replace(/[^a-zá-úñ0-9]+/gi, " ") + " ";
      for (const p of productos) {
        if (!p.matched || !p.product_id) continue;
        const actual = allProducts.find(x => String(x.id) === String(p.product_id));
        const yaLoTiene = ((actual?.variables as Array<{ options?: Array<{ name: string }> }>) || [])
          .some(g => (g.options || []).some(o => {
            const k = String(o.name || "").toLowerCase().trim();
            return k && voz.includes(" " + k + " ");
          }));
        if (yaLoTiene) continue;                       // el producto elegido ya encaja
        for (const [sabor, duenos] of dueno) {
          if (duenos.length !== 1 || duenos[0] === String(p.product_id)) continue;
          if (!voz.includes(" " + sabor + " ")) continue;
          const iOtro = allProducts.findIndex(x => String(x.id) === duenos[0]);
          if (iOtro < 0) continue;
          const rehecho = matchProducto(
            { n: iOtro + 1, nombre: String(allProducts[iOtro].name || ""),
              tamano: p.tamano, tipo: sabor, cantidad: p.cantidad, notas: p.notas },
            allProducts, allMods, allCats, clienteTexto, (cfgDomiRow?.[0] as Record<string, unknown>) || null);
          if (rehecho && rehecho.matched) { Object.assign(p, rehecho); }
          break;                                       // un solo cambio por producto
        }
      }
    }

    /* (3) AGENTE SUPERVISOR. No cambia nada: compara y avisa. Era la queja de
       Sergio — que cuando fallaba, fallaba callado. */
    let revision: Record<string, unknown> = { ok: true, problemas: [] };
    if (productos.length) {
      const resumen = productos.map(p =>
        "- " + p.cantidad + "x " + String(p.product_name || "") +
        (((p.adiciones as ModOpt[]) || []).length ? " + adicion: " + ((p.adiciones as ModOpt[]) || []).map(a => a.name).join(", ") : "") +
        (p.notas ? " [nota: " + p.notas + "]" : "")).join(_NL);
      const rev = await _agente(
        'Tu UNICA tarea: revisar. Abajo esta el pedido que armo el sistema y lo que escribio el cliente. Devuelve un JSON asi: {"correcto":true|false,"problemas":["..."]}.' +
        _NL + "- Marca correcto=false SOLO si algo no cuadra con lo que pidio: falta un producto, sobra uno, el tamaño o el tipo no son los que dijo, o el plato es otro." +
        _NL + "- Fijate especialmente en si el pedido tiene un plato mas caro del que el cliente nombro." +
        _NL + "- No inventes problemas de estilo. Si refleja lo que pidio: correcto=true y problemas vacio." +
        _NL + _NL + "PEDIDO ARMADO:" + _NL + resumen,
        "LO QUE ESCRIBIO EL CLIENTE:" + _NL + _vozCliente, 250);
      const okRev = rev.correcto !== false;
      const problemas = ((rev.problemas as string[]) || []).map(String);
      revision = { ok: okRev, problemas };
      /* Si el supervisor ve algo raro, los productos quedan MARCADOS para que el
         operador confirme antes de guardar. */
      if (!okRev) for (const p of productos) p.precio_confirmar = true;
    }
