        ...
        <button class="lm-prod${window.posStock ? ' ' + posStock.cardClass(p.id) : ''}" data-add="${p.id}">
          ...
          <div class="tp-prod-foot">
            ...
            <div class="tp-prod-row">
              <span class="tp-prod-price">${(window.posCarta && posCarta.precioEtiqueta ? posCarta.precioEtiqueta(p, fmt) : fmt(p.price)) || ''}</span>
...
async function loadCatalog() {
  ...
  function vrBuildVarPane(p, v) {
    ...
    const _presAgo=(VR_WIP.pres&&VR_WIP.pres.id&&VR_WIP.pres.id!=='_base')?VR_WIP.pres.id:null;
    const _ago={
    ...
    return '<div style="display:flex;flex-direction:column;gap:20px"><div>'
      ...
      +'<div class="pm-choice-grid">' + (v.options||[]).map(o => {
          ...
          return '<button class="pm-choice compact'+(sel?' on':'')+_ago.clase(o.id)+'" data-var-id="'+vrAttr(v.id)+'" data-opt-id="'+vrAttr(o.id)+'" data-opt-name="'+vrAttr(o.name)+'" data-opt-price="'+optPrice+'">'
