...
async function processConversation(convId: string, relectura = false): Promise<void> {
  ...
  isOpen = true;   // BANCO: se finge abierto
  ...
  if (false) { await setTyping(convId, false); return; }  // BANCO: contesta siempre
...
async function createWhatsappOrder(
  ...
  /* BANCO DE PRUEBAS: aqui produccion crea el pedido de verdad. En el banco no
     se escribe NADA en ventas — se devuelve un id falso y el flujo sigue igual,
     asi se puede probar la conversacion completa hasta el final sin ensuciar
     las ventas del restaurante. */
  console.log("[BANCO] pedido NO creado (simulado):", JSON.stringify(orderRecord).slice(0, 300));
  return "banco-" + String(convId).slice(0, 8);
