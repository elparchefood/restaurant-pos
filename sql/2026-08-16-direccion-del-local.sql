-- La direccion del local, en la pagina de clientes (16-ago).
--
-- La pantalla "El local" ya sabia pintar el bloque "Donde estamos" con el enlace
-- al mapa, pero salia vacio: fn_web_publica nunca mandaba la direccion.
--
-- La direccion NO vive en `tenants` (esa tabla ni siquiera tiene columna de
-- direccion): vive en la SUCURSAL (`branches.address` / `city` / `country`), que
-- es lo correcto — un restaurante con dos sedes tiene dos direcciones.
-- Se toma la sucursal activa mas antigua, la misma regla con la que ya se elige
-- la marca y el logo, para no inventar un criterio distinto.

drop function if exists public.fn_web_publica(text);

create function public.fn_web_publica(p_slug text)
returns table(tenant_id uuid, nombre text, abierto boolean, motivo text,
              detalle text, abre text, cierra text, permite_programar boolean,
              niveles jsonb, horarios jsonb, logo text, pago jsonb,
              direccion text, ciudad text, pais text)
...
grant execute on function public.fn_web_publica(text) to anon, authenticated;
