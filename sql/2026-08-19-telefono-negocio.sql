-- El telefono de la sucursal viaja a la pagina del cliente: es a donde va el
-- boton de escribirle al restaurante desde el seguimiento del pedido.
CREATE OR REPLACE FUNCTION public.fn_web_publica(p_slug text)
 RETURNS TABLE(tenant_id uuid, nombre text, abierto boolean, motivo text, detalle text, abre text, cierra text, permite_programar boolean, niveles jsonb, horarios jsonb, logo text, pago jsonb, direccion text, ciudad text, pais text, destacados jsonb, banner jsonb, telefono text)
 LANGUAGE plpgsql
