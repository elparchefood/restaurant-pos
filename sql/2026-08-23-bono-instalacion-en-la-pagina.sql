begin;
drop function if exists public.fn_web_publica(text);
CREATE OR REPLACE FUNCTION public.fn_web_publica(p_slug text)
 RETURNS TABLE(tenant_id uuid, nombre text, abierto boolean, motivo text, detalle text, abre text, cierra text, permite_programar boolean, niveles jsonb, horarios jsonb, logo text, pago jsonb, direccion text, ciudad text, pais text, destacados jsonb, banner jsonb, telefono text, bono_instalacion numeric)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT t0.id, t0.web_destacados, t0.web_banner,
         coalesce(t0.web_bono_instalacion, 0) AS bono,
  ...
  SELECT nullif(btrim(b.phone), '')   AS phone,
         nullif(btrim(b.address), '') AS address,
   ...
   LIMIT 1;
  /* EL NUMERO DEL RESTAURANTE, para que el cliente pueda escribirle por
     WhatsApp desde el seguimiento de su pedido. Sale de la sucursal, igual
     que la direccion. */
  telefono  := s.phone;
  direccion := s.address;
  ciudad    := s.city;
  ...
  /* EL REGALO POR INSTALAR LA APP (23-ago-2026, pedido de Sergio: que la
     pantalla del tutorial lo recalque). Sale de la BASE y por restaurante
     —tenants.web_bono_instalacion, el MISMO numero que ya usa web-acceso para
     entregarlo— y no escrito en la pantalla: si estuviera en dos sitios, el dia
     que Sergio lo cambie la pagina seguiria prometiendo el viejo, y eso es
     plata. 0 = no hay bono y la pantalla no promete nada. */
  bono_instalacion := coalesce(t.bono, 0);
