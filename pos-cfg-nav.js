...
(function (w) {
  ...
  { g: 'Ventas', s: 'dian', t: 'Facturación DIAN', p: 'dian',
  ...
  { g: 'Ventas', s: 'domicilios', t: 'Domicilios',
    i: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="5.5" cy="17.5" r="3.5"/><circle cx="18.5" cy="17.5" r="3.5"/><path d="M15 17.5h-6l-2-8h-3"/><path d="M9.5 9.5h6l1.5 5"/></svg>' },
  ...
  { g: 'Equipo', s: 'puntos', t: 'Puntos', p: 'puntos',
  ...
  { g: 'Chat IA', s: 'chatia', t: 'Asistente IA', p: 'chat_ia',
  ...
  function render(activa) {
    ...
    ITEMS.forEach(function (it) {
      ...
      html += '<button class="lm-nav' + (it.s === activa ? ' on' : '') + '"'
            ...
            /* `p` es la funcion del plan que abre esta seccion. pos-plan le
               pone el candado y, al tocarla, explica en que plan viene. La
               entrada NO se esconde: un boton ausente no le vende nada a
               nadie, un candado si. */
            + (it.p ? ' data-plan="' + it.p + '"' : '') + '>'
    ...
    if (grupo !== null) html += '</div>';
    nav.innerHTML = html;
    /* El menu se pinta cada vez; los candados hay que volver a ponerlos porque
       el innerHTML se los llevo. */
    try { if (w.posPlan && w.posPlan.marcarNav) w.posPlan.marcarNav(); } catch (e) {}
    return nav;
  }
