...
| 55 | 2026-07-31 | **Niveles de cliente (Estándar / Premium / VIP) con experiencia y caducidad (commits `d0c17fd`, `02136e4`):** el cálculo vive 100% en la BD — `fn_nivel_cliente(tenant, tel)` — para que el drawer del chat y la futura pantalla de registro del cliente lean exactamente el mismo número. Config en `pos_niveles_config` (por sucursal): `criterio` (`gastado`|`pedidos`|`puntos`), `xp_por_peso` (**0.0428**), `xp_por_pedido` (**137**), `caduca_meses` (**6**) y `niveles` jsonb `[{nombre, desde, min_pedidos, color}]`. **Dos cosas SEPARADAS, no confundirlas:** (a) la EXPERIENCIA es lo que se MUESTRA — `XP = floor(gastado × 0.0428) + pedidos × 137`; (b) el NIVEL se gana con **DINERO REAL** (`min_gastado` en el jsonb). **Ninguna bonificación de experiencia sube de nivel** (Sergio: *"por más bonificaciones aparentes que den experiencia, en las cuentas reales tiene que haber gastado 1000000 y 3000000"*). Verificado: $2.999.000 en 300 pedidos = 169.457 XP y **sigue Premium**, mientras que $3.000.000 en 60 pedidos = 136.620 XP y **sí es VIP**. **Topes vigentes:** Estándar $0 · Premium **$1.000.000** · VIP **$3.000.000**. La barra avanza según el DINERO (no la XP), así llega al 100% justo cuando cumple el monto y nunca antes; y el "faltan N XP" se calcula del dinero que falta × el factor, para que llegue a 0 en el mismo momento. El campo `desde` del jsonb quedó solo como referencia para criterios distintos de `gastado`. **Por qué el factor es raro y hay bono por pedido:** con 0.01 y topes de 10.000/30.000 la equivalencia era una regla de tres obvia (Sergio: *"se ve muy obvio… quisiera que sea más diferente la proporción para que no se note"*). Con DOS variables la XP deja de ser invertible: $1.000.000 da 44.170 XP en 10 pedidos y 48.280 XP en 40, así que nadie deduce el gasto mirando la barra. El bono además premia la frecuencia. Antes se habían puesto 1.500/5.000 XP; Sergio los corrigió: *"150000 pesos una persona lo puede gastar incluso en un solo día… sería ilógico que un nivel como el premium se obtenga en un día"*. **La barra NUNCA muestra dinero**, solo XP: el gasto real lo ve el negocio en las estadísticas de arriba del drawer. **Caducidad:** a los `caduca_meses` sin un solo pedido el cliente vuelve a Estándar desde cero. Implementado bien: la XP **no** suma sobre todo el historial sino sobre la **racha activa** — si alguna vez hubo un hueco > 6 meses entre pedidos, todo lo anterior a ese hueco ya caducó y no vuelve a contar (si no, un cliente que reaparece tras un año recuperaría VIP con un solo pedido). El drawer avisa "Conserva su nivel N meses más" y en ámbar "Pierde su nivel en N días" cuando faltan ≤45 (momento ideal para mandarle una promo). `min_pedidos` por nivel queda implementado y en 0: es la puerta para exigir además un mínimo de visitas si algún día se bajan los topes. Archivos: `chat-ia.js` v=1787120000 (`pintarFichaCliente`), `chat-ia.css` v=54 (`.ci-dw-nivcad`). |
