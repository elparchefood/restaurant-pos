...
async function loadProducts() {
  try {
    ...
    const cols = 'id,name,description,price,price_mode,category_id,available,photo_url,presentations,variables,mod_group_ids,mod_group_pres';
    ...
    if(error||!data){
      ...
      const r2 = await sb.from('pos_products').select('id,name,description,price,price_mode,category_id,available,presentations,variables,mod_group_ids,mod_group_pres').eq('tenant_id',S.tenantId).order('name');
...
function checkBaseHint(desc){
  const hasBase=/\bbase\b/i.test(desc||'');
  const badge=document.getElementById('base-badge-btn');
  if(badge)badge.style.display=hasBase?'inline-flex':'none';
  const hint=document.getElementById('base-type-hint');
  if(!hint)return;
  if(!hasBase){hint.style.display='none';if(S.editProd)S.editProd._baseHintDismissed=false;return;}
  if(S.editProd&&S.editProd._baseHintDismissed)return;
  const existing=S.bases||[];
  let inner='<span style="font-size:11.5px;color:#7C3AED;font-weight:700;flex-shrink:0">Base detectada →</span>';
  if(existing.length){
    inner+=existing.map(b=>'<button class="cc-base-hint-btn" onclick="selectBaseFromHint(\''+escHtml(b.name)+'\')">' +icon('layers',12)+' Usar: '+escHtml(b.name)+'</button>').join('');
    inner+='<button class="cc-base-hint-btn" onclick="selectBaseFromHint(\'__new__\')">'+icon('plus',12)+' Crear nueva</button>';
  } else {
    inner+='<button class="cc-base-hint-btn" onclick="selectBaseFromHint(\'__new__\')">'+icon('layers',12)+' Crear base de ingredientes</button>';
  }
  inner+='<button class="cc-base-hint-close" onclick="dismissBaseHint()" title="Ignorar">&#x2715;</button>';
  hint.innerHTML=inner;hint.style.display='flex';
}
function dismissBaseHint(){if(S.editProd)S.editProd._baseHintDismissed=true;const h=document.getElementById('base-type-hint');if(h)h.style.display='none';}
function selectBaseFromHint(n){dismissBaseHint();openBaseModal(n==='__new__'?'Base':n);}
...
function setProdDesc(v){S.editProd.desc=v;checkBaseHint(v);}
...
function refreshPresList(){const el=$('pres-list');if(!el)return;el.innerHTML=S.editProd.presentations.map(_presRowHTML).join('');}
// Fila de presentación con miniatura de imagen. Si la presentación no tiene
// imagen propia, al tomar el pedido usa la del producto (respaldo automático).
function _presRowHTML(pr){
  const total=(S.editProd&&S.editProd.presentations)?S.editProd.presentations.length:1;
  const img=pr._preview||pr.image_url;
  const thumb=img
    ? '<button type="button" onclick="triggerPresPhoto(\''+pr.id+'\')" title="Cambiar imagen de esta presentación" style="width:38px;height:38px;border-radius:9px;border:1px solid #E2E8F0;flex-shrink:0;position:relative;cursor:pointer;overflow:hidden;padding:0;background:#F1F5F9 center/cover no-repeat url(\''+escHtml(img)+'\')"><span onclick="event.stopPropagation();clearPresPhoto(\''+pr.id+'\')" title="Quitar (usar la del producto)" style="position:absolute;top:-2px;right:-2px;width:16px;height:16px;border-radius:999px;background:#0F172A;color:#fff;font-size:11px;line-height:16px;text-align:center;font-weight:700">&times;</span></button>'
...
function triggerPresPhoto(id){S._presUploadId=id;const inp=$('pres-photo-input');if(inp){inp.value='';inp.click();}}
function handlePresPhotoFile(inp){const f=inp.files[0];if(!f||!f.type.startsWith('image/'))return;const pr=S.editProd.presentations.find(x=>x.id===S._presUploadId);if(!pr)return;pr._imageFile=f;const r=new FileReader();r.onload=e=>{pr._preview=e.target.result;refreshPresList();};r.readAsDataURL(f);}
function clearPresPhoto(id){const pr=S.editProd.presentations.find(x=>x.id===id);if(!pr)return;pr.image_url=null;pr._preview=null;pr._imageFile=null;refreshPresList();}
...
// Precio en matriz (variante × presentación): actualiza o.prices[idx], que es lo
// que realmente usa el pedido. (Bug viejo: el input escribía en o.price y se perdía.)
function setVarMatrixPrice(vid,oid,idx,val){
  const vg=(S.editProd.variables||[]).find(x=>x.id===vid);if(!vg)return;
  const o=(vg.options||[]).find(x=>x.id===oid);if(!o)return;
  if(!Array.isArray(o.prices))o.prices=[];
  const n=(S.editProd.presentations||[]).length||1;
  while(o.prices.length<n)o.prices.push(0);
  o.prices[idx]=val;
  updatePriceRange();updateSaveProdBtn();
}
// HTML de una tarjeta de variable — con MATRIZ de precios si el producto es matrix
function _varCardHTML(v){
  const p=S.editProd;
  const pres=p.presentations||[];
  const isMatrix=p.priceMode==='matrix'&&v.isPricing&&pres.length>1;
  const head='<div class="cc-var-head"><span class="cc-var-tag">'+icon('sliders',12)+' Elección única</span><input class="cc-input flat" style="flex:1;font-weight:700" value="'+escHtml(v.name)+'" placeholder="Nombre" oninput="setVar(\''+v.id+'\',\'name\',this.value)"><button class="cc-mini-del" onclick="delVar(\''+v.id+'\')">'+icon('trash',13)+'</button></div>';
  let body;
  if(isMatrix){
    const cols='1fr '+pres.map(function(){return '84px';}).join(' ')+' 26px';
    const colHead='<div style="display:grid;grid-template-columns:'+cols+';gap:6px;align-items:center;font-size:9.5px;font-weight:700;color:#94A3B8;text-transform:uppercase;padding:2px 2px 4px"><span>Opción</span>'+pres.map(function(pr){return '<span style="text-align:center">'+escHtml(pr.name||'—')+'</span>';}).join('')+'<span></span></div>';
    const rows=v.options.map(function(o){
      const cells=pres.map(function(pr,idx){return '<div class="cc-money" style="margin:0"><span class="cc-money-sym">$</span><input type="number" min="0" step="500" value="'+((o.prices&&o.prices[idx])||'')+'" placeholder="0" oninput="setVarMatrixPrice(\''+v.id+'\',\''+o.id+'\','+idx+',parseInt(this.value)||0)"></div>';}).join('');
      return '<div style="display:grid;grid-template-columns:'+cols+';gap:6px;align-items:center;margin-top:6px"><input class="cc-input flat" value="'+escHtml(o.name)+'" placeholder="Opción" oninput="setVarOpt(\''+v.id+'\',\''+o.id+'\',\'name\',this.value)">'+cells+'<button class="cc-mini-del" '+(v.options.length===1?'disabled':'')+' onclick="delVarOpt(\''+v.id+'\',\''+o.id+'\')">'+icon('x',13)+'</button></div>';
    }).join('');
    body='<div style="margin-top:10px">'+colHead+rows+'</div>';
  }else{
    body='<div style="display:flex;flex-direction:column;gap:6px;margin-top:10px">'+v.options.map(function(o){return '<div class="cp-pres-row"><span class="cc-var-dot"></span><input class="cc-input flat" style="flex:1" value="'+escHtml(o.name)+'" placeholder="Opción" oninput="setVarOpt(\''+v.id+'\',\''+o.id+'\',\'name\',this.value)"><div class="cc-money"><span class="cc-money-sym">$</span><input type="number" min="0" step="500" value="'+(o.price||'')+'" placeholder="0" oninput="setVarOpt(\''+v.id+'\',\''+o.id+'\',\'price\',parseInt(this.value)||0)"></div><button class="cc-mini-del" '+(v.options.length===1?'disabled':'')+' onclick="delVarOpt(\''+v.id+'\',\''+o.id+'\')">'+icon('x',13)+'</button></div>';}).join('')+'</div>';
  }
  return '<div class="cc-var-card">'+head+body+'<button class="lm-link" style="margin-top:8px" onclick="addVarOpt(\''+v.id+'\')">+ Agregar opción</button></div>';
}
function refreshVarList(){const el=$('var-list');if(!el)return;if(!S.editProd.variables.length){el.innerHTML='<button class="cc-add-group" onclick="addVar()">'+icon('sliders',15)+' Agregar una variable (ej. Proteína: Pollo / Carne / Mixta)</button>';return;}el.innerHTML=S.editProd.variables.map(_varCardHTML).join('');}
// ── Modificadores por presentación ──────────────────────────────────────────
// Un grupo puede aplicar a todas las presentaciones (por defecto) o solo a
// algunas. Se guarda en p.modGroupPres[gid] = [presId,...]; ausente/vacío = todas.
function isModPresOn(p, gid, presId){
  const list=p.modGroupPres&&p.modGroupPres[gid];
  if(!list||!list.length) return true;   // todas
  return list.indexOf(presId)!==-1;
}
function _modRowsHTML(){
  const p=S.editProd;
  if(!S.mods.length) return '<button class="cc-add-group" onclick="openModEditorInProduct()">'+icon('plus',15)+' Crear el primer grupo de modificadores</button>';
  const presList=(p.presentations||[]).filter(pr=>(pr.name||'').trim());
  const multiPres=presList.length>1;
  return S.mods.map(g=>{
    const on=(p.modGroupIds||[]).includes(g.id);
    let presSel='';
    if(on&&multiPres){
      const chips=presList.map(pr=>{
        const pon=isModPresOn(p,g.id,pr.id);
        return '<button type="button" onclick="toggleModGroupPres(\''+g.id+'\',\''+pr.id+'\')" style="font-size:11.5px;font-weight:600;padding:4px 9px;border-radius:999px;cursor:pointer;display:inline-flex;align-items:center;gap:4px;border:1.5px solid '+(pon?'#5B6BFF':'#E2E8F0')+';background:'+(pon?'#EEF2FF':'#fff')+';color:'+(pon?'#4338CA':'#64748B')+'">'+(pon?icon('check',11,3):'')+escHtml(pr.name)+'</button>';
      }).join('');
      presSel='<div style="display:flex;flex-wrap:wrap;align-items:center;gap:6px;padding:2px 12px 10px 40px"><span style="font-size:11px;color:#94A3B8;font-weight:700">Aplica en:</span>'+chips+'</div>';
    }
    return '<div style="display:flex;flex-direction:column">'
      +'<button class="cc-modtoggle'+(on?' on':'')+'" onclick="toggleModGroup(\''+g.id+'\')"><span class="cc-check'+(on?' on':'')+'">'+(on?icon('check',12,3):'')+'</span><div style="flex:1;min-width:0;text-align:left"><div style="display:flex;align-items:center;gap:7px"><span style="font-size:13px;font-weight:700;color:#0F172A">'+escHtml(g.name)+'</span><span class="cc-tiny-chip">'+RULE_LABEL[g.rule]+'</span><span class="cc-tiny-chip">'+(g.multi?'Varias':'Una')+'</span></div><div style="font-size:11px;color:#94A3B8;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+g.options.map(o=>o.name).join(' · ')+'</div></div><span style="font-size:10.5px;font-weight:700;color:#94A3B8;flex-shrink:0">'+g.options.length+' opc.</span></button>'
      +presSel
      +'</div>';
  }).join('');
}
function refreshModList(){const el=$('mod-list');if(el)el.innerHTML=_modRowsHTML();}
function toggleModGroup(gid){
  const p=S.editProd; const ids=p.modGroupIds||[];
  if(ids.includes(gid)){ p.modGroupIds=ids.filter(x=>x!==gid); if(p.modGroupPres) delete p.modGroupPres[gid]; }
  else { p.modGroupIds=[...ids,gid]; }
  refreshModList();
}
function toggleModGroupPres(gid, presId){
  const p=S.editProd;
  const allIds=(p.presentations||[]).filter(pr=>(pr.name||'').trim()).map(pr=>pr.id);
  p.modGroupPres=p.modGroupPres||{};
  let list=(p.modGroupPres[gid]&&p.modGroupPres[gid].length)?p.modGroupPres[gid].slice():allIds.slice();
  list=list.indexOf(presId)!==-1?list.filter(x=>x!==presId):list.concat([presId]);
  if(!list.length||list.length>=allIds.length) delete p.modGroupPres[gid]; // vacío o todas → aplica a todas
  else p.modGroupPres[gid]=list;
  refreshModList();
}
// Limpia el mapa para guardar: solo grupos activos y solo cuando es un
// subconjunto real de presentaciones (si son todas, se omite = compatibilidad).
function _cleanModGroupPres(p){
  const map=p.modGroupPres||{}; const ids=p.modGroupIds||[];
  const presIds=(p.presentations||[]).map(pr=>pr.id);
  const out={};
  ids.forEach(function(gid){
    const list=(map[gid]||[]).filter(function(id){return presIds.indexOf(id)!==-1;});
    if(list.length&&list.length<presIds.length) out[gid]=list;
  });
  return out;
}
...
async function saveProduct(){
  ...
  // Imágenes por presentación (opcionales). Se suben al bucket; NUNCA base64 en BD.
  const _pid=p.id||uid('p');
  for(const pr of (p.presentations||[])){
    if(pr._imageFile){ const url=await uploadPhoto(pr._imageFile,_pid+'_'+pr.id,'presentations'); if(url) pr.image_url=url; }
    delete pr._imageFile; delete pr._preview;   // no persistir datos temporales
  }
...
function renderModEditor(){
  ...
  const optRows=g.options.map(o=>'<div class="cp-pres-row"><span class="cc-grip">'+icon('grip',14)+'</span><input class="cc-input flat" value="'+escHtml(o.name)+'" placeholder="Ej. Queso fundido" style="flex:1" oninput="setModOpt(\''+o.id+'\',\'name\',this.value)"><div class="cc-money"><span class="cc-money-sym">$</span><input type="number" min="0" step="500" value="'+(o.prices&&o.prices.length?Math.min(...o.prices)||'':o.price||'')+'" placeholder="0" oninput="setModOpt(\''+o.id+'\',\'price\',parseInt(this.value)||0)"></div><button class="cc-mini-del" '+(g.options.length===1?'disabled':'')+' onclick="delModOpt(\''+o.id+'\')">'+icon('x',13)+'</button></div>').join('');
...
function renderCatEditor(){
  ...
  openOverlay('<div class="cc-overlay center" onmousedown="handleOverlayClose(event)"><div class="cc-modal narrow" onmousedown="event.stopPropagation()"><div class="cc-modal-head"><div style="display:flex;align-items:center;gap:10px"><span class="cc-modal-glyph" style="color:'+c.color+';background:'+c.tint+'">'+icon('layers',16)+'</span><div><div style="font-size:10px;color:#94A3B8;text-transform:uppercase;letter-spacing:.08em;font-weight:700">'+(isNew?'Nueva categoría':'Editar categoría')+'</div><div style="font-size:16px;font-weight:800;color:#0F172A;letter-spacing:-.02em">'+(escHtml(c.name)||'Sin nombre')+'</div></div></div><button class="lm-icon-sm" onclick="closeOverlay()">'+icon('x',15)+'</button></div><div class="cc-modal-body"><input type="file" id="cat-photo-input" accept="image/*" style="display:none" onchange="handleCatPhotoFile(this)">'+catPhotoHTML+'<label><span class="field-label">Nombre de la categoría</span><input class="cc-input" value="'+escHtml(c.name)+'" placeholder="Ej. Salchipapas" oninput="setCatName(this.value)" autofocus></label><label style="display:block;margin-top:14px"><span class="field-label">Nombre en comanda <span style="font-weight:400;color:#94A3B8">(opcional · singular)</span></span><input class="cc-input" value="'+escHtml(c.comanda_alias||'')+'" placeholder="Ej. Hamburguesa, Perro" oninput="setCatAlias(this.value)"></label><div style="font-size:11px;color:#94A3B8;margin-top:5px;line-height:1.45">Se usa en la comanda cuando el producto no tiene nombre de presentación. Si lo dejas vacío, se usa el nombre de la categoría.</div><div style="margin-top:16px"><div style="font-size:10.5px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:.05em;margin-bottom:7px">Color de etiqueta</div><div style="display:flex;gap:10px;flex-wrap:wrap">'+palBtns+'</div></div><div class="cc-preview-bar"><span style="font-size:11px;color:#94A3B8;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Vista previa</span><span id="cat-preview-tag" class="cc-cat-tag-preview" style="color:'+c.color+';background:'+c.tint+'">'+(escHtml(c.name)||'Categoría')+'</span></div></div><div class="cc-modal-foot"><span></span><div style="display:flex;gap:8px"><button class="lm-btn-ghost" onclick="closeOverlay()">Cancelar</button><button class="lm-btn-primary" id="save-cat-btn" '+(c.name.trim()?'':'disabled')+' onclick="saveCat()">'+icon('check',14)+' Guardar</button></div></div></div></div>');
}
...
function setCatAlias(v){S.editCat.comanda_alias=v;}
