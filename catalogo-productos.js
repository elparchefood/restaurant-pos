...
const CAT_PALETTE = [
  ...
  {color:'#A31621',tint:'#FBEAEC',ring:'#EBB9BE'},
...
function renderBaseModal(){
  ...
  openOverlay(
    ...
    +'<span class="cc-drawer-glyph" style="color:#C0463C;background:#F5F3FF">'+icon('layers',17)+'</span>'
...
function openBaseAssign(){
  ...
  const rows=candidates.length?candidates.map(p=>{
    ...
    return '<label style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #F1F5F9;cursor:pointer">'
      +'<input type="checkbox" id="ba-'+p.id+'" '+(checked?'checked':'')+' style="width:16px;height:16px;accent-color:#C0463C">'
  ...
  openOverlay(
    ...
    +'<div class="cc-drawer-head">'
    +'<div style="display:flex;align-items:center;gap:10px">'
    +'<span class="cc-drawer-glyph" style="color:#C0463C;background:#F5F3FF">'+icon('users',17)+'</span>'
...
function renderAIImport(){
  ...
  if(stage==='source'){
    ...
    bodyHTML='<div style="padding:8px 0">'
      ...
      +'<span style="font-size:10.5px;font-weight:800;color:#C0463C;text-transform:uppercase;letter-spacing:.07em">Paso '+stepNum+' de 4 — '+stepLabel+'</span>'
      +'<span style="font-size:12px;font-weight:800;color:#C0463C">'+progress+'%</span>'
      +'</div>'
      +'<div style="height:7px;background:#F1F5F9;border-radius:99px;overflow:hidden;margin-bottom:20px">'
      +'<div style="height:100%;width:'+progress+'%;background:linear-gradient(90deg,#C0463C,#A31621);border-radius:99px;transition:width .9s ease"></div>'
      +'</div>'
    ...
    const catBlocks=ex.categories.map((cat,ci)=>{
      ...
      const prodRows=open&&!off?cat.products.map((pr,pi)=>{
        ...
        if(isEditing){
          ...
          const varInputs=(pr.variables||[]).map((v,vi)=>{
            ...
            return '<div style="margin-bottom:8px"><div style="font-size:10px;font-weight:700;color:#C0463C;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Variable: '+escHtml(v.name)+'</div>'
              +opts
          ...
          return '<div class="cc-prod-row" style="flex-direction:column;align-items:stretch;gap:6px;padding:10px 0">'
            ...
            +'<span style="font-size:12px;font-weight:700;color:#C0463C;flex:1">'+escHtml(pr.name)+'</span>'
        ...
        const vRows=(pr.variables||[]).map(v=>'<div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap"><span style="font-size:9.5px;font-weight:800;color:#C0463C;text-transform:uppercase;letter-spacing:.05em;white-space:nowrap">'+escHtml(v.name)+':</span>'+v.options.map(o=>'<span class="cc-pres-tag" style="background:#F5F3FF;color:#7C3AED;border:1px solid #DDD6FE">'+escHtml(o.name)+'</span>').join('')+'</div>').join('');
        ...
        const varChip=pr.variables&&pr.variables.length?'<span style="font-size:9px;font-weight:700;background:#F5F3FF;color:#C0463C;padding:2px 6px;border-radius:99px">'+pr.variables.length+' var.</span>':'';
      ...
      return '<div class="cc-cat-block" style="opacity:'+(off?.55:1)+';border-color:'+(open&&!off?'#DDD6FE':'#ECEEF2')+'">'
        ...
        +'<button class="cc-check'+(off?'':' on')+'" data-cat="'+escHtml(cat.name)+'" onclick="toggleAICat(this.dataset.cat)" style="'+(off?'':'background:#C0463C;border-color:#C0463C;color:#fff')+'">'+(off?'':icon('check',12,3))+'</button>'
