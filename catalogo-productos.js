...
function renderCatEditor(){
  ...
  openOverlay('<div class="cc-overlay center" onmousedown="handleOverlayClose(event)"><div class="cc-modal narrow" onmousedown="event.stopPropagation()"><div class="cc-modal-head"><div style="display:flex;align-items:center;gap:10px"><span class="cc-modal-glyph" style="color:'+c.color+';background:'+c.tint+'">'+icon('layers',16)+'</span><div><div style="font-size:10px;color:#94A3B8;text-transform:uppercase;letter-spacing:.08em;font-weight:700">'+(isNew?'Nueva categoría':'Editar categoría')+'</div><div style="font-size:16px;font-weight:800;color:#0F172A;letter-spacing:-.02em">'+(escHtml(c.name)||'Sin nombre')+'</div></div></div><button class="lm-icon-sm" onclick="closeOverlay()">'+icon('x',15)+'</button></div><div class="cc-modal-body"><input type="file" id="cat-photo-input" accept="image/*" style="display:none" onchange="handleCatPhotoFile(this)">'+catPhotoHTML+'<label><span class="field-label">Nombre de la categoría</span><input class="cc-input" value="'+escHtml(c.name)+'" placeholder="Ej. Salchipapas" oninput="setCatName(this.value)" autofocus></label><div style="margin-top:16px"><div style="font-size:10.5px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:.05em;margin-bottom:7px">Color de etiqueta</div><div style="display:flex;gap:10px;flex-wrap:wrap">'+palBtns+'</div></div><div class="cc-preview-bar"><span style="font-size:11px;color:#94A3B8;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Vista previa</span><span id="cat-preview-tag" class="cc-cat-tag-preview" style="color:'+c.color+';background:'+c.tint+'">'+(escHtml(c.name)||'Categoría')+'</span></div></div><div class="cc-modal-foot"><span></span><div style="display:flex;gap:8px"><button class="lm-btn-ghost" onclick="closeOverlay()">Cancelar</button><button class="lm-btn-primary" id="save-cat-btn" '+(c.name.trim()?'':'disabled')+' onclick="saveCat()">'+icon('check',14)+' Guardar</button></div></div></div></div>');
}
