...
function productCardHTML(p){
  ...
  return '<div class="cp-card" style="'+(p.active?'':'opacity:.72')+'" onclick="openEditor(\''+p.id+'\',\'product\')">'+'<div class="cp-thumb">'+thumb+'<span class="cp-cat-chip" style="color:'+cat.color+';background:'+cat.tint+'">'+escHtml(cat.name)+'</span>'+inactive+'</div>'+'<div class="cp-card-body"><div class="cp-card-row"><div class="cp-card-name">'+escHtml(p.name)+'</div><div class="cp-card-price">'+range+'</div></div>'+(p.desc?'<div class="cp-card-desc">'+escHtml(p.desc)+'</div>':'')+'<div class="cp-meta-row"><span class="cp-meta-tag">'+icon('layers',12)+' '+p.presentations.length+' '+(p.presentations.length===1?'presentación':'presentaciones')+'</span>'+varTag+'<span class="cp-meta-tag">'+icon('tag',12)+' '+(groups.length?(groups.length+' '+(groups.length===1?'grupo':'grupos')+' · '+opts+' adic.'):'Sin adiciones')+'</span></div></div>'+'<div class="cp-card-foot" onclick="event.stopPropagation()"><button class="cp-switch'+(p.active?' on':'')+'" onclick="toggleProduct(\''+p.id+'\')"><span class="cp-switch-lbl">'+(p.active?'Activo':'Inactivo')+'</span><span class="cp-switch-track"><span class="cp-switch-knob"></span></span></button><div style="display:flex;gap:6px;align-items:center"><button class="cp-card-del-btn" onclick="event.stopPropagation();confirmDeleteProduct(\''+p.id+'\')">'+icon('trash',13)+'</button><button class="cp-card-edit-btn" onclick="openEditor(\''+p.id+'\',\'product\')">Editar '+icon('chevron',13)+'</button></div></div></div>';
}

function confirmDeleteProduct(id){
  const p=S.products.find(x=>x.id===id);if(!p)return;
  openOverlay('<div class="cc-overlay center" onmousedown="handleOverlayClose(event)"><div class="cc-modal" style="width:360px;max-width:90vw" onmousedown="event.stopPropagation()"><div class="cc-modal-head" style="border-bottom:none;padding-bottom:6px"><div style="display:flex;align-items:center;gap:10px"><span style="width:38px;height:38px;border-radius:10px;background:#FFF1F2;color:#F43F5E;display:flex;align-items:center;justify-content:center">'+icon('trash',16)+'</span><div style="font-size:14px;font-weight:800;color:#0F172A">Eliminar producto</div></div><button class="lm-icon-sm" onclick="closeOverlay()">'+icon('x',15)+'</button></div><div style="padding:4px 20px 20px"><div style="font-size:13px;color:#475569;line-height:1.55">¿Eliminar <strong>'+escHtml(p.name)+'</strong>?<br>Esta acción no se puede deshacer.</div><div style="display:flex;gap:8px;margin-top:16px"><button class="lm-btn-ghost" style="flex:1" onclick="closeOverlay()">Cancelar</button><button style="flex:1;background:#EF4444;border:none;border-radius:10px;color:#fff;font-size:13px;font-weight:700;height:38px;cursor:pointer;" onclick="doDeleteProduct(''+p.id+'')">Eliminar</button></div></div></div></div>');
}
