...
function checkBaseHint(desc){
  ...
  if(!hint)return;
  if(!hasBase){hint.style.display='none';return;}
  const existing=S.bases||[];
  const curId=S.editProd?S.editProd.id:null;
  const cur=existing.find(b=>Array.isArray(b.product_ids)&&b.product_ids.includes(curId));
  const opts='<option value="">Elegir base…</option>'
    +existing.map(b=>'<option value="'+escHtml(b.name)+'"'+(cur&&cur.name===b.name?' selected':'')+'>'+escHtml(b.name)+'</option>').join('')
    +'<option value="__new__">＋ Crear base nueva</option>';
  hint.innerHTML=
    '<span class="base-picker-ic">'+icon('layers',14)+'</span>'
    +'<span class="base-picker-lbl">Base</span>'
    +'<div class="base-picker-sel"><select onchange="if(this.value)selectBaseFromHint(this.value)">'+opts+'</select></div>'
    +(cur?'<button class="base-picker-edit" onclick="openBaseModal(\''+escHtml(cur.name)+'\')" title="Ver / editar ingredientes de esta base"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button>':'');
...
function renderPageHead(){
  ...
  if(isProd&&S.selectMode){
    ...
    actions.innerHTML=search
      ...
      /* Solo asoma si esta sede tiene precios propios que restablecer. */
      +((isProd&&window.cpMultiSede&&cpMultiSede()&&posCarta.cuantosAjustados())
        ?'<button class="lm-btn-ghost sm" onclick="cpRestablecerCarta()">Restablecer carta de '+escHtml(cpSucNombre())+' ('+posCarta.cuantosAjustados()+')</button>':'')
...
// ── Product Editor ────────────────────────────────────────────────────────
/* Las cuatro medallas, con el MISMO color que van a tener en la pagina. Los
...
function openEditor(id,type){
  if(type==='product'){
    ...
    S.editProd=existing?JSON.parse(JSON.stringify(existing)):{id:uid('p'),cat:(S.cats[0]||{}).id||'_',name:'',desc:'',active:true,photo:null,presentations:[{id:uid('pr'),name:'',price:0}],variables:[],modGroupIds:[],medalla:'',_photoFile:null};
...
function renderProductEditor(){try{
  ...
  /* LAS MEDALLAS de la pagina de clientes. Se ven con su color de verdad, no
     como una lista de palabras: la gracia de la medalla es el color, y elegir
