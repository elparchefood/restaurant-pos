...
| 26 | 18-ago | Juan Sebastian | Dijo "por nequi o llave" y el pedido quedo con el pago cargado al SALDO del cliente ("billetera el parche food") | El lector recibe los metodos con el nombre pelado; el saldo se llama "Billetera El Parche Food" y se llevaba cualquier billetera digital | Cada metodo va con lo que de verdad es + regla explicita + candado deterministico si el texto nombra nequi/llave/QR. 5 de 5 en el banco | 202 |
| 27 | 18-ago | (queja de la noche) | "con pocas salsas" y "con un billete de 100" no llegaron a la comanda; ademas le cobraron una adicion Salsa de $2.000 | El disparador exigia "poca" y la ese del plural lo tapaba; el cambio del domiciliario no se capturaba; y el nombre precedido de un "menos" entraba como adicion | Terminacion suelta en los disparadores, captura del billete/sencillo, y compuerta pideMenosDe: pedir menos no es pedir mas | 202 |
| 28 | 18-ago | Monica R. | "maicitos especial de pollo" sumaba una adicion Pollo de $9.000 — y en las pruebas "mixta personal" salio con adiciones Carne y Pollo ($19.000 de mas) | El lector EXPLICA lo que significa la variante y esa explicacion entraba cobrada | Una opcion de variante solo entra como adicion si el cliente la nombro | 202 |
| 29 | 18-ago | (hallado en pruebas) | "salchipapa premium personal" (sin decir sabor) salia como Premium MIXTA, y el sabor decide el precio | El lector inventaba la variante y el tamano, y la validacion solo comprobaba que existieran en la carta | Misma compuerta de rastro que las adiciones: sin rastro en el texto se descarta y Paco PREGUNTA | 202 |
| 30 | 18-ago | (hallado en pruebas) | "...con adicion de pollo PARA RECOGER" le pedia la direccion; y en "premium de carne y UNA MIXTA personal" se perdia una linea entera | El limpiador recorta en "para " y dejaba "recoger" suelto (que ya no coincide); y la mixta se descartaba como si fuera el sabor de la primera | Canonico "Para recoger" para cualquier forma suelta + un articulo delante vuelve la mencion plato propio | 202 |
| 31 | 18-ago | Ivan | Le contestaron la MISMA frase de prepago tres veces seguidas ignorando "¿y no se puede en efectivo?" | Las tres ramas que mandan esa frase respondian y salian, sin llevar cuenta de cuantas veces ya lo habian dicho | A la segunda vez pasa a una persona. La cuenta vive en su propia columna (chat_conversations.bucles): dentro de pending_order_data se borraba sola | 207 |
| 32 | 18-ago | (hallado en pruebas) | "salchipapa mixta personal" salio con adiciones Carne y Pollo: $19.000 de mas. Tercera aparicion del mismo volcado | La compuerta solo miraba cuando venian 3 o mas adiciones, y este venia de a dos | Toda adicion tiene que dejar rastro en el texto del cliente, sin excepcion por cantidad; los sinonimos entran por tolerancia de 2 letras | 207 |
| 33 | 18-ago | (hallado probando) | "quitame la tocineta" y "quitame la coca cola" no hacian nada si se decian ANTES del resumen: la adicion seguia cobrada y el producto seguia en el pedido | Quitar solo se atendia dentro del bloque del resumen, y es justo antes cuando mas se dice | Quitar vale en cualquier momento, y tambien saca de la cola los platos que aun no han entrado | 210 |
| 34 | 18-ago | (hallado probando) | "mejor mandalo a la calle 15, barrio Bella Vista" tras el resumen: Paco CONTESTABA QUE SI y el pedido conservaba direccion, barrio y domicilio viejos | No existia rama para cambiar la direccion despues del resumen | Se reemplaza y se limpia lo que depende (barrio y precio del domicilio se recalculan). Si el pedido YA salio a cocina, va a una persona en vez de fingir que lo cambio | 210 |
| 35 | 18-ago | (hallado probando) | "agregame una coca cola personal" tras el resumen: Paco REDACTABA un resumen con la gaseosa y el pedido no la tenia. El total decia $31.000 y la comanda llevaba solo la salchipapa | La puerta de "agregar" solo abria si el lector devolvia `producto`, y tras el resumen casi nunca lo devuelve | Tambien abre cuando el CATALOGO reconoce un plato nuevo en el mensaje | 210 |
| A2 | 15-ago | — | (riesgo latente, cerrado el 18-ago) `resolverPedido` de verify-transfer tenia su PROPIO matching sin los tres desempates de la entrada 140 | La comanda podia salir con el nombre y el precio de la categoria equivocada | Los tres desempates portados. Y de paso: "Salchipapas TrADICIONales" contenia "adicion" y toda la categoria estaba clasificada como adiciones | 208 |
