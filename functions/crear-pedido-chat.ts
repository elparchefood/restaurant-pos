  ...
  try {
    ...
    try {
      if (telefono) {
        ...
        const cands = await sbGet(`/rest/v1/pos_clientes?tenant_id=eq.${tenantId}&telefono=like.*${encodeURIComponent(tel10)}&select=id,nombre,direccion,direcciones`) as Array<Record<string, unknown>> | null || [];
        ...
        if (existente) {
          ...
          if (direccion) {
            ...
            const previas = Array.isArray(existente.direcciones) ? (existente.direcciones as string[]) : [];
            const todas = previas.slice();
            if (existente.direccion && !todas.some((d) => nn(d) === nn(existente.direccion))) todas.push(String(existente.direccion));
            if (!todas.some((d) => nn(d) === nn(direccion))) todas.push(direccion);
            patch.direcciones = todas;
            patch.direccion = direccion;
          }
          await sbPatch(`/rest/v1/pos_clientes?id=eq.${existente.id}`, patch);
        } else {
          const nc = await sbPost(`/rest/v1/pos_clientes`, { tenant_id: tenantId || null, branch_id: branchId, nombre: cliente, telefono, direccion: direccion || null, direcciones: direccion ? [direccion] : [], updated_at: new Date().toISOString() }, true);
    ...
    const orderNotes = ((direccion || "") + barrioTag + telTag + (notas ? " — " + notas : "")).trim() || null;
